...
LuminatorBlock::LuminatorBlock():ICEntityBlock("te.luminator_flat",IC::Blocks::ID::mLuminator,Material::getMaterial(MaterialType::LAMP))
{
	init();
...
bool LuminatorBlock::use(Player&p, BlockPos const&pos)const
{
	ICPE::mUIScreenChooser.pushLuminatorScreen(p,pos);
	return true;
}
ICBlockEntity* LuminatorBlock::getBlockEntity(BlockSource&s,BlockPos const&pos)const
{
	return new LuminatorBlockEntity(s,pos,this);
}
bool LuminatorBlock::isElectricBlock(BlockSource&s,BlockPos const&pos)const
{
	return true;
}
bool LuminatorBlock::isElectricReceiver(BlockSource&s,BlockPos const&pos,FacingEnum)const
{
	return true;
}
const AABB &LuminatorBlock::getVisualShape(BlockSource&source, BlockPos const&pos, AABB&, bool) const
{
...
const AABB &LuminatorBlock::getVisualShape(unsigned char, AABB&, bool) const
{
	return AABB(0,0.001,0,1,0.0625,1);
}
int LuminatorBlock::getPlacementDataValue(Entity&, BlockPos const&, signed char side, Vec3 const&, int) const
{
	return side+1;
}
#endif
