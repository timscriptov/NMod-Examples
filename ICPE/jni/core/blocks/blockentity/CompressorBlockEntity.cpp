#include "CompressorBlockEntity.h"
...
#include "items/recipe/CompressorRecipes.h"

const float CompressorBlockEntity::mSpeed=2.5E-3;
const unsigned long CompressorBlockEntity::energyPerTick=2;

bool CompressorBlockEntity::isEnergyReceiver(FacingEnum)const
{
	return true;
}
CompressorBlockEntity::CompressorBlockEntity(BlockPos const&pos):ElectricMachineBlockEntity(pos,3)
{
	burnProgress=0.0F;
	isLit_=false;
}
void CompressorBlockEntity::load(mca::CompoundTag const&tag)
{
...
void CompressorBlockEntity::save(mca::CompoundTag &tag)const
{
...
void CompressorBlockEntity::tick(Level&l,unsigned long long t)
{
	...
	if(burnProgress>=1.0F)
	{
		if(!getSlot(0)||getSlot(0)->isNull())
			goto burnEnd;
		ItemInstance item=CompressorRecipes::getInstance()->getResult(getSlot(0));
		if(item.isNull())
			goto burnEnd;
		...
		getSlot(0)->remove(CompressorRecipes::getInstance()->getFromCount(getSlot(0)));
		burnProgress=0.0F;
	}
...
bool CompressorBlockEntity::isLit()const
{
	return isLit_;
}
unsigned int CompressorBlockEntity::getIVoltage()const
{
	return 32;
}
void CompressorBlockEntity::setLit(bool b)
{
	isLit_=b;
	//update();
}
bool CompressorBlockEntity::canChargeWithDischargeable(ItemInstance const&item)
{
	if(!canCharge())
		return false;
...
bool CompressorBlockEntity::canPushInItem(int s,ItemInstance const&i)
{
...
float CompressorBlockEntity::getWorkProgress()const
{
	if(burnProgress>=0.0F)
		return burnProgress;
	return 0.0F;
}
float CompressorBlockEntity::getElectricProgress()const
{
	if(getEnergy()<=0)
		return 0.0F;
	return ((float)getEnergy())/((float)getEnergyStorage());
}
unsigned int CompressorBlockEntity::getItemVoltage(ItemInstance const&i)const
{
	return ItemUtils::getOVoltage(i);
}
unsigned long CompressorBlockEntity::getItemEnergyLast(ItemInstance const&i)const
{
	return ItemUtils::getEnergy(i);
}
bool CompressorBlockEntity::canBurn()
{
	if(getEnergy()<energyPerTick)
		return false;
	if(!getSlot(0)||getSlot(0)->isNull())
		return false;
	if(CompressorRecipes::mInstance->isCompressorItem(getSlot(0)))
	{
		if(getSlot(2)&&!getSlot(2)->isNull())
		{
			if(CompressorRecipes::getInstance()->getResult(getSlot(0)).sameItemAndAux(*getSlot(2))&&!getSlot(2)->isFullStack())
			{
				ItemInstance item=*getSlot(2);
				item.add(CompressorRecipes::getInstance()->getResult(getSlot(0)).count);
...
bool CompressorBlockEntity::canCharge()
{
...
unsigned long CompressorBlockEntity::getEnergyStorageDefault()const
{
	return 800;
}
